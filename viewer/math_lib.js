...
function closest(all_vecs, target_idx, distance_fn=cosine_d){
    console.assert(all_vecs.length > 2, "closest only works when number of vectors is at least 2")
    var closest_idx = target_idx == 0 ? 1 : 0;
    var closest_dist = 10e20;
    for(var i = 0; i < all_vecs.length; i++){
        var this_dist = cosine_d(all_vecs[i],all_vecs[target_idx]);
        if(i != target_idx && this_dist < closest_dist){
